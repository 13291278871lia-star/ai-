...
export function panel() {
 ...
 <section class="page panel"><h2>已接通的完整製作流程</h2><p>上傳計劃書後，平台會依次生成：概覽页、可编辑 IRR 折线图、价值里程碑、身故保障、保单延续安排、弹性工具、计算假设、缺失资料检查及来源核对。配图采用已接入的 AI 生成视觉素材。</p><ul><li>20／30／40／50年是保單年數；100歲須減去投保年齡。40歲投保到100歲＝60年。</li><li>人民幣＝美元×匯率，最後展示時才四捨五入。IRR保留完整精度，展示兩位小數。</li><li>提取總額＝每年提取×次數，連續年齡區間含首尾。倍數＝提取及期末剩餘價值÷累計保費，不能寫成年利率。</li><li>分清保證及非保證價值。每個期限分別計算一次退保，不能把多個期限的退保總額相加。</li><li>计划书没有的提款、预缴或推广资料会标示为未提取，不会沿用其他产品或模板旧数字。</li></ul><details><summary>原附件的差異與校驗記錄</summary><p>圖一為「財富盈活」、GF模版為「環宇盈活」、PDF為「財富恆裕3」。圖一100年行的325,921,240美元×6.8＝2,216,264,432人民幣，與圖中221,626,443不符；原資料尚未確定哪一欄有誤，因此不載入為保單數據。</p><p>IRR表「總」B36缺少正現金流，C54等空白輸入會除以0；平台遇到這些情況會顯示錯誤。模版第10頁部分人民幣數字使用7.25換算，與頁腳6.8不符，因此所有新概覽統一使用表單匯率。</p></details></section>`;
}
...
export function bind() {
 ...
 let profileExtra={};
 const get=()=>({...profileExtra,product:document.querySelector('#insProduct').value.trim(),age:Number(document.querySelector('#insAge').value),premium:Number(document.querySelector('#insPremium').value),years:Number(document.querySelector('#insYears').value),levy:Number(document.querySelector('#insLevy').value),fx:Number(document.querySelector('#insFx').value),source:document.querySelector('#insSource').value.trim(),rows:parseRows(document.querySelector('#insRows').value)});
 let last=null;
 const fields=[['insProduct','product'],['insAge','age'],['insPremium','premium'],['insYears','years'],['insLevy','levy'],['insFx','fx'],['insSource','source']];
 const render=()=>{const out=document.querySelector('#insuranceOutput');try{if([...document.querySelectorAll('#insuranceForm input[required]')].some(e=>!e.value.trim()))throw Error('請補齊產品、年齡、保費、供款年數、徵費及匯率');const c=get(), rows=calculate(c);last={c,rows};document.querySelector('#insError').textContent='';out.innerHTML=`<span class="tag">計算完成 · 11頁可編輯PPT</span><h2>${esc(c.product)}</h2><p>${c.age}歲投保 · ${c.years===1?'整付保費':c.years+'年供款'} · 匯率 ${c.fx}</p><div class="insurance-live-chart">${chartSvg(c,rows)}</div><p class="tiny">折線圖與下表同步計算；圖中年度按實際間距排列。</p><div class="insurance-table-wrap"><table class="insurance-table"><thead><tr><th>保單年末</th><th>美元</th><th>人民幣</th><th>年化IRR</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r.year}年<br><small>${r.age}歲</small></td><td>${money(r.usd)}</td><td>${money(r.rmb)}</td><td>${(r.rate*100).toFixed(2)}%</td></tr>`).join('')}</tbody></table></div><p class="tiny">退保總額可能含非保證分紅。IRR計入保費及所填額外支出，並非保證利率。各行為獨立退保情景。</p><p class="tiny">來源：${esc(c.source)}</p><button class="primary" id="downloadInsurance">下載完整模版 PPT（含表格＋折線圖）↓</button> <button class="primary secondary" id="downloadCalcDeck">下載計算版（僅表格＋IRR圖）↓</button> <button class="primary secondary" id="downloadCalculation">下載計算明細 CSV ↓</button><p id="insuranceDownloadStatus" role="status"></p><details><summary>查看退保價值組成</summary>${rows.map(r=>`<p>${r.year}年：${r.guaranteed===null?"未提供保證／非保證拆分，合計 ":"保證 "+money(r.guaranteed)+" + 非保證 "+money(r.bonus)+" = "}${money(r.usd)} 美元</p>`).join('')}</details>`;
 document.querySelector('#downloadInsurance').onclick=async()=>{const b=document.querySelector('#downloadInsurance'),notice=document.querySelector('#insuranceDownloadStatus');b.disabled=true;notice.textContent='正在生成完整模版 PPT…';try{const resp=await fetch('/api/ppt-from-template',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({profile:c,rows})});if(!resp.ok){const t=await resp.text();throw new Error(t||('HTTP '+resp.status));}const blob=await resp.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=c.product.replace(/[\\/:*?"<>|]/g,'_')+'_完整模版.pptx';a.click();setTimeout(()=>URL.revokeObjectURL(url),2000);notice.textContent='已下載完整模版 PPT（已填表格＋可編輯折線圖）。';}catch(e){notice.textContent='下載失敗：'+e.message+'（後端需安裝 python-pptx：pip install python-pptx）';}finally{b.disabled=false;}};
 document.querySelector('#downloadCalcDeck').onclick=async()=>{const b=document.querySelector('#downloadCalcDeck'),notice=document.querySelector('#insuranceDownloadStatus');b.disabled=true;try{await createDeck(window.PptxGenJS,c,rows).writeFile({fileName:c.product.replace(/[\\/:*?"<>|]/g,'_')+'_計算版.pptx'});notice.textContent='已下載計算版 PPT（pptxgenjs 生成，僅表格＋IRR圖）。';}catch(e){notice.textContent='下載失敗：'+e.message;}finally{b.disabled=false;}};
 ...
 const apply=c=>{profileExtra=Object.fromEntries(Object.entries(c||{}).filter(([k])=>!['product','age','premium','years','levy','fx','source','rows'].includes(k)));for(const [id,k]of fields)document.getElementById(id).value=c[k]??'';document.querySelector('#insRows').value=c.rows?.map(r=>r.join(',')).join('\n')??'';render();};
